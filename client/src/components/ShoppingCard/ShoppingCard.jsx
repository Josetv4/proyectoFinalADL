...
const ShoppingCard = ({ toggleDrawer }) => {
  ...
  if (
    !cartsAllbyUser ||
    !cartsAllbyUser.carts ||
    !Array.isArray(cartsAllbyUser.carts)
  ) {
    return (
      <div>
        <h1 >No has realizado compras</h1>
        <div>
          ...
          <Card key={`${cart.product_id}${index}`} className="shopping-card">
            <CardContent className="shopping-card-content">
              ...
              <Typography variant="p">
                {new Date(cart.created_at).getDate()} de{" "}
                {new Date(cart.created_at).toLocaleString("es", {
                  month: "long",
                })}{" "}
                del {new Date(cart.created_at).getFullYear()}
              </Typography>
            </CardContent>
